...
public class EvidenceMessage extends GeneralMessage {
	...
	protected void initInstance(REMMDMessageType _evidence) {
		...
		if (acceptanceRejection != null) {
			addAcceptanceRejectionByRecipient(
					new AcceptanceRejectionByRecipient(acceptanceRejection),
					true);
		}
		...
		if (deliveryNonDelivery != null) {
			addDeliveryNonDeliveryToRecipient(
					new DeliveryNonDeliveryToRecipient(deliveryNonDelivery),
					true);
		}
	...
	protected void addRelayToREMMDAcceptanceRejection(
		...
		if (parseObject) {
			if (_relayToREMMDAcceptanceRejection
					.getXSDObject()
					.getEventCode()
					.equals(SpocsConstants.Evidences.RELAY_REM_MD_ACCEPTANCE_REJECTION
							.getFaultEventCode())) {
				isSuccessfull = false;
	...
	protected void addRelayToREMMDFailure(
		...
		if (parseObject) {
			if (_relayToREMMDFailure
					.getXSDObject()
					.getEventCode()
					.equals(SpocsConstants.Evidences.RELAY_REM_MD_FAILURE
							.getFaultEventCode())) {
				isSuccessfull = false;
				evidenceException = new RelayToREMMDFailureException(
						_relayToREMMDFailure,
						REMErrorEvent
								.getRemErrorEventForJaxB(_relayToREMMDFailure
										.getXSDObject().getEventReasons()),
						null);
	...
	protected void addDeliveryNonDeliveryToRecipient(
		...
		if (parseObject) {
			if (_deliveryNonDeliveryToRecipient
					.getXSDObject()
					.getEventCode()
					.equals(SpocsConstants.Evidences.DELIVERY_NON_DELIVERY_TO_RECIPIENT
							.getFaultEventCode())) {
	...
	protected void addRetrievalNonRetrievalByRecipient(
		...
		if (parseObject) {
			if (_retrievalNonRetrievalByRecipient
					.getXSDObject()
					.getEventCode()
					.equals(SpocsConstants.Evidences.RETRIEVAL_NON_RETRIEVAL_BY_RECIPIENT
							.getFaultEventCode())) {
				isSuccessfull = false;
	...
	protected void addAcceptanceRejectionByRecipient(
		...
		if (parseObject) {
			if (_acceptanceRejectionByRecipient
					.getXSDObject()
					.getEventCode()
					.equals(SpocsConstants.Evidences.ACCEPTANCE_REJECTION_BY_RECIPIENT
							.getFaultEventCode())) {
				isSuccessfull = false;
	 ...
	 * @throws SpocsConfigurationException
	 *             if the gateway is configured wrong
	 */
	@Override
				...
				JaxbContextHolder
						...
						.marshal(
								new org.etsi.uri._02640.soapbinding.v1_.ObjectFactory()
										.createREMMDMessage(getXSDObject()),
								out);
	...
	protected void sendEvidenceMessage(Evidence evidenceObj)
		...
		synchronized (EvidenceMessage.class) {
			...
			try {
				...
				HttpsURLConnection
						.setDefaultSSLSocketFactory(new HTTPSTransport(tslData)
								.createSSLFactory());
				service = getService(new URL(supplyPoint),
