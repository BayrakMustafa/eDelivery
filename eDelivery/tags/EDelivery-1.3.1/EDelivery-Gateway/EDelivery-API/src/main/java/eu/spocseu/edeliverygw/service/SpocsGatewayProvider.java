...
public class SpocsGatewayProvider implements Provider<Message> {
	...
	public Message invoke(Message soapMessage) {
		...
		if (action == null)
			throw throwSoapFault(
					"Could not parse the incoming spocs message no action found",
					SOAPConstants.SOAP_SENDER_FAULT, new NullPointerException());
	...
	private Message createResponseMsg(Element returnObject)
		...
		Message message = com.sun.xml.ws.api.message.Messages
				.createUsingPayload(new StreamSource(new ByteArrayInputStream(
						XMLSignatureHelper.dumpDomNode(returnObject)
								.toByteArray())),
